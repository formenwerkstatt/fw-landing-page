...
export default function ImageAccordion() {
  return (
    <div className="group mx-auto mb-10 mt-3 flex  justify-center gap-2 max-md:flex-col">
      {items.map((item) => {
        return (
          <article
            key={item.id}
          ...
          >
            <div className="absolute inset-0 z-10 flex  flex-col justify-end p-3 text-white">
              <h3 className=" text-xl font-medium   transition duration-200 ease-[cubic-bezier(.5,.85,.25,1.8)] group-focus-within/article:translate-y-0 group-focus-within/article:opacity-100 group-focus-within/article:delay-300 group-hover/article:translate-y-0 group-hover/article:opacity-100 group-hover/article:delay-300 md:translate-y-2 md:truncate md:whitespace-nowrap md:opacity-0">
                {item.title}
              </h3>
